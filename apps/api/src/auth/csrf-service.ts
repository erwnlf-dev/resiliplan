...
const EXEMPT_PATHS = new Set([
  '/api/v1/auth/login',
  '/api/v1/auth/logout',
  // External webhooks (signature-verified instead of CSRF)
  '/api/v1/webhooks/in/prometheus',
]);
...
export function shouldCheckCsrf(method: string, url: string): boolean {
  ...
  // All external inbound webhooks are signature-verified, not CSRF-protected
  if (path.startsWith('/api/v1/webhooks/in/')) return false;
  return true;
}
